...
public class RehomeSellSubmitActivity extends AppCompatActivity {
    ...
    EditText title, birth, content, sell, local;
    ...
    private void upload(){
        ...
        for (int i=0; i< uriList.size(); i++){
            if(!uriList.get(i).equals(null))
            {
                ...
                StorageReference postImg = ImgRef.child(document_id);
                UploadTask uploadTask = postImg.child(fileName).putFile(uriList.get(i));
    ...
    void showDialog() {
        AlertDialog.Builder msgBuilder = new AlertDialog.Builder(RehomeSellSubmitActivity.this).setMessage("게시글을 작성하시겠습니까?").setPositiveButton("확인", new DialogInterface.OnClickListener() {
            @Override
